...
public class ArrayDistribution extends Distribution {
	...
	public ArrayDistribution(int max, @NonNull IntToDoubleFunction distribution) {
		super(max);
	...
	private double initArrayWithFunction(IntToDoubleFunction distribution, Double[] prob) {
		double sum = 0.0;
		for (int i = 1; i <= prob.length; i++) {
			double p = distribution.applyAsDouble(i);
			if (p < 0.0) {
				throw new IllegalArgumentException(
						"Function " + distribution.getClass().getSimpleName() + " generate negativ probabilities");
			}
			prob[i - 1] = p;
			sum += p;
		}
	...
	private void addTheProbabilities(Double[] prob) {
		...
		if (kummulated[kummulated.length - 1] > 1.0001) {
			throw new IllegalArgumentException(
					"Summe der Wahrscheinlichkeiten sollte <= 1.0 sein ist aber " + kummulated[kummulated.length - 1]);
		}
	}
